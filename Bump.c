...
void BumpInt_Init(void(*task)(uint8_t)){
    ...
    NVIC->IP[9] = (NVIC->IP[9]&0x00FFFFFF)|0x04000000; // priority 2
...
void PORT4_IRQHandler(void){
    ...
    uint8_t sensorValue = Bump_Read();

    // Call the user-supplied collision handling function
     if (collisionHandler != 0) {
         collisionHandler(sensorValue);
     }

     Motor_Backward(7500,7500);
     //justin ~ direction based state machine
